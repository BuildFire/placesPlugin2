...
window.detailView = {
    init: (place) => {
      ...
      if (
        place &&
        place.hasOwnProperty("categories") &&
        window.app.state.configCategories == true
      )
        place.categories.map((item) => {
          categories.push(
            app.state.categories
              .filter((category) => category.name.id === item)
              .map((c) => c.name.name)
          );
        });
      ...
      window.buildfire.auth.getCurrentUser((err, user) => {
        context.actionItems =
          (place.actionItems && place.actionItems.length > 0) ||
          (window.app.state.chatWithLocationOwner &&
            window.app.state.socialWall &&
            window.app.state.socialWall.instanceId &&
            place.contactPerson &&
            place.contactPerson.id &&
            user &&
            place.contactPerson.id !== user._id);
        ...
        req.onload = () => {
            ...
            window.buildfire.geo.getCurrentPosition(null, (err, position) => {
              ...
              if (position && position.coords) {
                ...
                destinations.forEach((item) => {
                  ...
                  let distance = window.buildfire.geo.calculateDistance(
                    origin,
                    destination,
                    { decimalPlaces: 5 }
                  );
            ...
            Buildfire.getContext((err, context) => {
              if (
                ...
                Buildfire.navigation.openWindow(
                  `https://maps.apple.com/?q=${hbsContext.title}&t=m&daddr=${hbsContext.lat},${hbsContext.lng}`,
                  "_system"
                );
              } else {
                Buildfire.navigation.openWindow(
                  `http://maps.google.com/maps?daddr=${hbsContext.lat},${hbsContext.lng}`,
                  "_system"
                );
              }
            });
          ...
          function showContact() {
            ...
            window.buildfire.auth.getCurrentUser((err, user) => {
              const includesChat =
                actionItems.findIndex((item) => item.customChat) >= 0;
              if (user) {
                if (
                  context.chatWithLocationOwner &&
                  context.socialWall &&
                  place.contactPerson &&
                  place.contactPerson.id &&
                  user._id !== place.contactPerson.id &&
                  !includesChat
                ) {
                  let wid = "";
                  ...
                  const sendPNTo = JSON.stringify([
                    place.contactPerson.id,
                    user._id,
                  ]);
                  actionItems.push({
                    action: "linkToApp",
                    queryString: `wid=${wid}&wTitle=${encodeURIComponent(
                      (user.displayName || "Someone") +
                        " | " +
                        (place.contactPerson.displayName || "Someone")
                    )}&sendPNTo=${sendPNTo}`,
              ...
              window.buildfire.actionItems.list(
                actionItems,
                {},
                (err, actionItem) => {
          ...
          if (context.isBookmarkingAllowed) {
            document
              ...
              .addEventListener("click", handleBookmarkClicked);
            setBookmark();
          }

          function setBookmark() {
            let bookmarkButton = document.getElementById("bookmarkBtn");
            bookmarkButton.className = context.bookmarked
          ...
          function getBookmarks() {
            window.buildfire.bookmarks.getAll(function (err, bookmarks) {
              ...
              let bookmark = bookmarks.find(
                (bookmark) => bookmark.id === context.id
              );
              if (bookmark) {
          ...
          function handleBookmarkClicked() {
            window.buildfire.bookmarks.getAll(function (err, bookmarks) {
              ...
              if (bookmark) {
                window.buildfire.bookmarks.delete(
                  bookmark.id,
