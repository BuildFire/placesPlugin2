...
window.mapView = {
      ...
      if (!err && position && position.coords) {
        ...
        if (buildfire.isWeb()) {
          ...
          localStorage.setItem(
            "user_location",
            JSON.stringify(mapView.lastKnownLocation)
          );
          buildfire.spinner.hide();
        }
      }
    ...
    if (places && places.length) {
      ...
      if (places.length == 1) {
        google.maps.event.addListenerOnce(
          ...
          "bounds_changed",
          function (event) {
            if (this.getZoom()) this.setZoom(mapView.settings.zoomLevel.city);
          }
        );
      }
    }
  ...
  addMarkerCluster: () => {
    ...
    // Add a marker clusterer to manage the markers.
    mapView.settings.markerClusterer = new MarkerClusterer(
      map,
      app.state.markers,
      clusterOptions
    );
  ...
  addMarker: (map, place, iconType) => {
    ...
    if (place.address) {
      ...
      marker.addListener("click", () => {
        mapView.markerClick(place, marker);
      });
    }
  },
  markerClick: (place, marker) => {
    ...
    if (
      app.state.selectedPlace.length > 1 &&
      app.state.selectedPlace[1].marker
    ) {
      app.state.selectedPlace[1].marker.setIcon(
        mapView.createMarker(mapView.settings.images.place)
      );
    }
    let categories = [];
    ...
    req.onload = () => {
      ...
      closeDiv.onclick = (e) => {
        ...
        //Un-select location
        app.state.selectedPlace[0].marker.setIcon(
          mapView.createMarker(mapView.settings.images.place)
        );
  ...
  createMap: () => {
    ...
    let zoomTo =
        mapView.lastKnownLocation != defaultLocation
          ? mapView.settings.zoomLevel.city
          : mapView.settings.zoomLevel.country,
      centerOn =
        mapView.lastKnownLocation != defaultLocation
          ? mapView.lastKnownLocation
          : defaultLocation;
    ...
    window.originalHeight = app.views.mapView
